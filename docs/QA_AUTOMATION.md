...
- Whole-game sanity: title boots and is interactive, every `MapTransition` target scene and spawn marker exists in both directions across all seventeen rooms, every shipped dialogue line fits the real DialogueBox rect at the real font size, save/summary/reload round-trips, corrupt saves fail safely, every music context resolves to an existing stream (`GameSanityAudit.gd`).
- Route and quest integrity (`RequiredRouteStateSmoke.gd`, `QuestFlowAudit.gd`, `ScenePathSmoke.gd`, `StorylineSanitySmoke.gd`, `LoreConsistencySmoke.gd`). `QuestFlowAudit.gd` walks the required route and checks that quest data is complete and that RouteCue produces guidance from every room.
- UI and layout (`MinigameLayoutAudit.gd`, `MinigameUiArchitectureSmoke.gd`, `MinigamePauseCoverageSmoke.gd`, `PauseMenuSmoke.gd`, `PresentationConsistencySmoke.gd`, `SaveSlotDisplaySmoke.gd`). `MinigameLayoutAudit.gd` instantiates every catalogued minigame screen and measures visible labels and buttons against their parent rectangles, including alternate states and multi-line capacity; the rest cover shared UI structure, pause coverage on every screen, and slot-text layout.
- Dialogue data and presentation (`DialoguePoolSmoke.gd`, `DialoguePortraitSmoke.gd`, `DialogueStyleSmoke.gd`, `DialogueHandoffSmoke.gd`, `PostMinigameDialogueSmoke.gd`). `DialoguePortraitSmoke.gd` asserts that portrait sources stay at or above the displayed rect, so art is only ever downscaled.
- Descent-cue correctness in Static Service Depths (`StaticDescentCueSmoke.gd`): every arrow sits on a shelf and points at a drop that lands on another shelf; every climb gate stands on a shelf and lifts to one; and the objective order (spawn, each core, the exit) is reachable by dropping between shelves. That course authors no thresholds, so a cue aimed at a gap costs the player the whole descent.
...
- Per-stage regressions (`HybridExplorerSmoke.gd`, which covers the shared adventure movement FSM, `CircuitSodaSmoke.gd`, `TruthFilterSmoke.gd`, `BrokenHighScoreSmoke.gd`, `ArchiveHistorySmoke.gd`, `HallwayFlowSmoke.gd`, `OpeningArrivalSmoke.gd`, `PrizeEchoHandoffSmoke.gd`, `CircuitSodaStoryHandoffSmoke.gd`, `ClosingShiftEchoesSmoke.gd`, `UnknownVoiceMusicDuckSmoke.gd`, `DebugDiagnosticsSmoke.gd`, `NavigationUiSmoke.gd`).
