...
- `tools/RunRegressionSuite.ps1` is the maintained validation entry point: an editor-wide parse, a main-scene boot, then 31 checks from `scripts/qa/`.
...
- **QA runner crashes.** The old `user://logs` crash advice predates the regression suite, which runs 31 `--script` checks sequentially, each with its own `--log-file` and `--disable-crash-handler`.
