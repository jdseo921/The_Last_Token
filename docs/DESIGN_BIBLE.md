...
### 2.1 Tone rule for the dark past — **implied, poetic, hopeful** (locked)
...
### 3.1 The Motto (the spine of the whole game)
...
### 3.3 Delivery — seed throughout, **realize in the climax, collect in post-game**
...
## 5. Memory Signal System (canonical mapping)
...
## 6. Dialogue Rules (apply to every line written)
...
## 7. Story Flow (45–60 min)

The required spine is unchanged in *order* — it is coherent — but each beat now has an explicit **purpose, a foreshadow, an optional motto fragment, and a tension/breather rhythm.** Timings are budget targets to hold the 45–60 min total.

| # | Beat | ~min | Purpose | Foreshadow planted | Motto | Rhythm |
|---|---|---|---|---|---|---|
| 0 | **Wake / Intro** (ArcadeHub, meet Mira) | 3 | Hook: recognized, not arriving. Teach walk/talk/interact. | Mira's "the door had forgotten you"; Owner Portrait (dusty). | — | Calm |
| 1 | **Rockbyte Duel** (Cabinet 07) | 4 | First win; the machine knows his "signal." | Cabinet 07 "PREVIOUS PLAYER PROFILE FOUND." | **A** | Tension → breather (Mira's token anecdote) |
| 2 | **Truth Filter** (Mr. Byte) | 3 | Sorting his contradictory memories. → **Fractured** | "Identity conflict remains." First `???` after. | **A** | Tension |
| 3 | **Circuit Soda** (Vendo) | 4 | Route the fractured signal; kept without a label. | Vendo "old staff bought grape soda after closing." | **B** | Breather-heavy (comic relief after two puzzles) |
| 4 | **Lost Shift File** (read + deduce) | 4 | Discover a hidden final-night shift. | Staff Schedule "Employee ## — Cabinet shutdown"; number-before-name. | — (lore) | Investigative, quiet dread |
| 5 | **Static Service Run** (Gus) | 4 | Bring the dead service route back to life. | Maintenance Note "two signals after closing." | **B** | Tension → Gus breather ("scared and did the job anyway") |
| 6 | **Maintenance Sync** (Gus) | 3 | Line up the two door-signals. → **Overloaded** | "Door heard both knocks. Yours, and the one you forgot making." | — | Peak-ish tension |
| 7 | **Security Tape Assembly** | 3 | Prove the door recorded no *customer*. | "The Staff Door did not record a customer." The anomaly is 04. | — | Tension |
| 8 | **Final Night Walk** | 4 | Retrace the last night from memory. | "One walked in. Two signals answered." `???` after. | **B** | High tension, elegiac |
| 9 | **Memory Echo** | 3 | Anchor the real memories before the truth. | The motto fragments surface as "true" memories. | **both** | Quiet, tense |
| 10 | **Staff Room Reveal** | 6 | Slideshow → philosophical self-conflict (Player ↔ ???, **no names**) → motto as self-realization → ending. → **Restored** | Payoff of the internal arc. | **both** | Climax → quiet finality |
| 11 | **Post-reveal roam** | opt | Witnesses remember 04; the warm **collection** of fragments + hopeful callbacks land here. | Redeems NPC coverage. | **both** | Release |
| — | Inter-beat travel, NPC side-talk, conscience interludes | ~8–10 | Breathing room, optional fragments, world texture. | Owner Portrait / Broken Cabinet escalation. | mixed | — |

**Budget check:** stages ≈ 30–35 min + travel/talk/intro/ending ≈ 15–20 min ⇒ **~45–55 min required**, landing in-target with headroom. Optional games (Broken High Score, Prize Sort, +) are opt-in and additive. **Do not over-write per-stage dialogue** — the required route already nearly fills the budget on its own.

---

## 8. Open design calls (flagged for your input)

These are the judgment calls made while reworking the docs. Each is my recommendation; say the word to change any.

1. **Optional-game consolidation (my main recommendation / criticism).** There are currently **four near-identical "adventure" maze reskins** (`HubTicketSweep`, `CabinetTraceRun`, `SnackServiceDash`, `PrizeShelfRun`) all running one engine — this is the exact bloat you were wary of, and only two optional games (Broken High Score, Prize Sort) are actually wired to quests. I recommend a **lean, distinct optional roster** instead of four samey mazes. Proposed set in [`STAGE_DESIGN.md` §4](STAGE_DESIGN.md). *This is a "major change" — your call.*
2. **Difficulty now lives in code, not labels.** The `target_minutes` config is dead (never read) and the two maze stages are mechanically identical; the redesign makes escalation real. Detailed in `STAGE_DESIGN.md`.
3. **Ending reprise name-checks fragments the player saw.** Small conditional-dialogue addition so the climax gathers *your* run's fragments. Optional to implement; strong payoff.
...
Purely technical docs stay live: `BUILD_NOTES.md`, `QA_AUTOMATION.md`, `FLAG_REGISTRY.md`, `CONTROLS.md`, `TEST_PLAN.md`, and the art docs (to be reworked in the later visual pass).
