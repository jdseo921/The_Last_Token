...
func _init() -> void:
	...
	_expect_player_text("mira quest comedy", DIALOGUE_POOL.get_lines("mira", "lost_token_quest_instruction"), "(I walk into a closed arcade, get recognized by strangers, and immediately receive an errand. Curiosity has consequences.)")
	...
	_expect_player_text("vendo circuit comedy", DIALOGUE_POOL.get_lines("vendo", "circuit_soda_intro"), "(A vending machine missed me. I do not understand any of this. Questioning it can wait; I should keep moving.)")
	...
	_expect_contains_text("vendo prize service directions", DIALOGUE_POOL.get_lines("vendo", "unknown_voice_clue"), "Prize Service Hall is the passage on the right, between Circuit Soda and me.")
	...
	_expect_first_text("cabinet 07 fractured", DIALOGUE_POOL.get_lines("cabinet_07", "fractured_echo"), "CABINET STATUS: RESTLESS.")
	...
	_expect_first_text("reel first meeting", DIALOGUE_POOL.get_lines("reel", "first_meeting"), "Oh. A request. Been a while since anyone worked my buttons.")
	_expect_first_text("reel memory echo intro", DIALOGUE_POOL.get_lines("reel", "memory_echo_intro"), "This is the last set. The one the arcade never got to finish.")
	_expect_first_text("reel post reveal", DIALOGUE_POOL.get_lines("reel", "post_reveal_witness"), "Employee 04. The one who taped songs over the ad reel.")
	_expect_first_text("coily first meeting", DIALOGUE_POOL.get_lines("coily", "first_meeting"), "WIND ME UP! ...oh. You actually did.")
	_expect_first_text("coily security tape intro", DIALOGUE_POOL.get_lines("coily", "security_tape_intro"), "Ooh, home movies! I love home movies!")
	_expect_first_text("coily post reveal", DIALOGUE_POOL.get_lines("coily", "post_reveal_witness"), "Employee 04! The one who rewired my GAME OVER into halftime!")
	...
	_expect_first_text("Gus continuous service intro", DIALOGUE_POOL.get_lines("gus", "static_service_full_intro"), "You made it. Broken Score and Service Dash agree that the shutdown ended here.")
	...
	_expect_contains_text("environment ticket fractured", DIALOGUE_POOL.get_lines("environment_objects", "ticket_counter_fractured"), "For a moment it does not move when you move.")
	...
	_expect_first_text("environment employee file", DIALOGUE_POOL.get_lines("environment_objects", "employee_04_file_archived"), "EMPLOYEE 04 // MEMORY ACCESS: SEALED.")
